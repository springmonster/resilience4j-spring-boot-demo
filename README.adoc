...
The demo shows how to use the `CircuitBreaker` annotation to make your Spring Boot application more fault tolerant.
...
But you can still use a functional programming style.
For example:
...
#There are mistakes in this part#

The emitted CircuitBreaker events are stored in a separate circular event consumer buffers.
The size of a event consumer buffer can be configured per CircuitBreaker in the application.yml file (eventConsumerBufferSize).
